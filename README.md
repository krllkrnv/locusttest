...
- `loadtest/` — сценарии Locust, скрипты запуска тестов, HTML/CSV артефакты прогонов.
...
По совокупности наблюдений наиболее вероятным ограничением является доступ к файловому хранилищу (`data/terms.json`) при параллельных запросах (синхронное чтение/запись и конкуренция за I/O). Сеть на результаты почти не влияет, так как все компоненты работали на localhost.

Важно: это гипотеза на основании метрик Locust (рост p95/p99 без ошибок) и устройства хранилища. Без мониторинга CPU/Disk I/O и/или профилирования обработчиков “узкое место” фиксируется только косвенно.
...
- Если оставлять файл: не писать в файл на критическом пути ответа (очередь/отложенная запись), добавить кэш чтения и корректную блокировку записи
